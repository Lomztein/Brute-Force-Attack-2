...
{
    public class TurretAssemblyModuleOrbEffect : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                Assign(GetComponentInParent<TurretAssemblyModule>());
            }
            UpdateTrailPosition();
        }

        public void Assign (TurretAssemblyModule module)
        {
            Structure structure = module.GetComponentInParent<Structure>();
            _hoverDistance = new Vector2(World.Grid.SizeOf(structure.Width), World.Grid.SizeOf(structure.Height)).magnitude;
            Color color = Colorization.ColorInfo.Get(module.Item.Color).DisplayColor;
